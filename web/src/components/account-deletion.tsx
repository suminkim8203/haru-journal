...
export function AccountDeletion({onClose,onRequested}:{onClose:()=>void;onRequested:(status:DeletionStatus)=>void}){
 ...
 return <section className="account-panel"><div className="account-actions"><h2>계정 탈퇴</h2><button onClick={onClose} disabled={busy} aria-label="탈퇴 화면 닫기">닫기</button></div><p>신청 즉시 모든 기기에서 로그아웃되며 기록 접근이 중단됩니다. 7일 안에 다시 로그인해 탈퇴를 취소할 수 있습니다.</p><p>7일 후 계정과 계획·할 일·실행 기록·단상·회고를 삭제하며 복구할 수 없습니다. 필요한 기록은 먼저 내려받으세요.</p><button onClick={download} disabled={busy}>내 기록 내보내기</button><p className="auth-help">7일의 유예기간이 끝나면 운영 자료와 관리 중인 백업 사본의 기록을 순차적으로 삭제합니다. 백업에 별도의 추가 유예기간을 두지 않습니다. 장애가 발생하면 처리가 지연될 수 있습니다.</p><form onSubmit={submit}><label className="auth-field">현재 비밀번호<input type="password" autoComplete="current-password" required value={password} onChange={e=>setPassword(e.target.value)} disabled={busy}/></label><label className="account-confirm"><input type="checkbox" checked={confirmed} onChange={e=>setConfirmed(e.target.checked)} disabled={busy}/>7일 이후 계정과 기록이 삭제되는 것을 확인했습니다.</label>{message&&<p role="alert" className="auth-feedback">{message}</p>}<button className="auth-primary" disabled={busy||!confirmed||!password}>{busy?'처리 중…':'탈퇴 신청'}</button></form></section>;
}
