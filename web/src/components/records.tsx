...
export function ReflectionEditor({day,saved,onSaved,onCancel,closing=false}:{day:string;saved?:Reflection;onSaved:()=>void;onCancel?:()=>void;closing?:boolean}){
 ...
 return <form className="reflection-editor" onSubmit={async e=>{e.preventDefault();if(await send(closing?'close_day':'save_reflection',{date:day,body,imports}))onSaved();}}><label>오늘은 어떤 하루였나요?<textarea className="record-input" required={!closing} maxLength={4000} value={body} onChange={e=>setBody(e.target.value)} placeholder={closing?"기억하고 싶은 순간이나 마음에 남은 생각을 적어 주세요. 회고를 작성하지 않아도 하루를 마무리할 수 있습니다.":"기억하고 싶은 순간이나 마음에 남은 생각을 적어 주세요."}/></label>
...
export function DayClosing({day}:{day:string}){
 ...
 const saved=data?.reflections.find(r=>r.local_date===day),unfinished=data?.tasks.filter(t=>!t.complete&&!t.skipped&&!t.cancelled_at&&t.due_date===day)||[];
...
export function Archive({onDay}:{onDay:(day:string)=>void}){
 const {data,locked,send}=useJournal();const [tab,setTab]=useState<'reflections'|'trash'>('reflections'),[bookmarks,setBookmarks]=useState(false),[date,setDate]=useState(''),[query,setQuery]=useState(''),[restoreOverlap,setRestoreOverlap]=useState<Record<string,boolean>>({});
 ...
 {tab==='trash'&&<><p className="field-help">삭제한 기록을 복원할 수 있습니다. 자동으로 완전히 삭제하지 않습니다.</p>{data?.trash.map(t=><article className="trash-row" key={t.id}><div><h3>{t.title}</h3><p className="field-help">{({plan:'계획',task:'할 일',run:'실행 기록',placement:'계획 배치',reflection:'회고',routine:'반복 예외'})[t.entity_type]} · {t.deleted_at.slice(0,10)} 삭제</p></div><div>{t.placement_conflict&&<label className="complete-control"><input type="checkbox" checked={!!restoreOverlap[t.id]} onChange={e=>setRestoreOverlap(v=>({...v,[t.id]:e.target.checked}))}/>예정 시간 겹침을 확인했으며 그대로 복원</label>}<button disabled={locked||!!t.placement_conflict&&!restoreOverlap[t.id]} onClick={()=>void send('restore_entity',{trashId:t.id,allowOverlap:!!restoreOverlap[t.id]})}>복원</button></div></article>)}{!data?.trash.length&&<p className="empty">휴지통이 비어 있습니다.</p>}</>}
 </>;
}
