...
export function RoutineEditor({plan,rule,task,onClose}:{plan:Plan;rule?:Routine;task?:Task;onClose:()=>void}){
 ...
 <p className="form-help">과거·완료·실행 이력·개별 수정·건너뛰기·중단 항목을 보존합니다. 실제 기록은 덮어쓰지 않습니다.</p>
 {(impact?.expands||draft.startDate<plan.start_date!||draft.endDate>plan.end_date!)&&<p className="form-help">부모 계획의 기간도 함께 확장하고 변경 전 계획을 보존합니다. 자동으로 축소하지 않습니다.</p>}
 <details><summary>적용 날짜와 보존 이유</summary><ul>{impact?.targets.slice(0,50).map(t=><li key={t.id}>{t.date} · 수정</li>)}{impact?.addedDates.map(d=><li key={d}>{d} · 추가</li>)}{impact?.removed.slice(0,50).map(t=><li key={t.id}>{t.date} · 기간 제외</li>)}{impact?.protectedRows.slice(0,50).map(t=><li key={t.id}>{t.date} · 보존: {t.reason}</li>)}</ul><p className="form-help">각 목록은 처음 50개를 표시합니다. 위 개수는 전체 적용 대상입니다.</p></details>
 ...
 <div className="actions">{review&&<button type="button" className="link" onClick={()=>setReview(false)}>내용으로 돌아가기</button>}<button type="submit" className="btn primary" disabled={review&&scope!=='one'&&!impact?.valid}>{review?'변경 확정':'적용 대상 확인'}</button></div>
 </fieldset></form>;
}
export function RoutineRules({plan}:{plan:Plan}){
 ...
 return <section><div className="section-heading"><h2 data-typo-role="title">반복 할 일 {rules.length}개</h2><button disabled={locked||creating||!!editing} className="btn primary" onClick={()=>{setCreating(true);setEditing(null);}}>반복 할 일 추가</button></div><p className="form-help">날짜별 완료와 단상은 일정의 일간에서 확인합니다.</p>
 ...
 {rules.map(r=><article className="run-row" key={r.id}><h4 className="task-title" data-typo-role="title">{r.title}</h4><p className="form-help">{r.start_date}–{r.end_date} · 매일 {r.estimated_minutes}분 · {r.timing==='fixed'?r.start_time?.slice(0,5):'시간 미정'}{r.stopped_from?' · '+r.stopped_from+'부터 중단':''}</p><div className="side-actions"><button className="link" disabled={locked||creating} onClick={()=>setEditing(r)}>반복 규칙 수정</button><button className="link" disabled={locked||!!r.stopped_from} onClick={()=>{setStopping(r);setFrom(seoulDate()<r.start_date?r.start_date:seoulDate()>r.end_date?r.end_date:seoulDate());}}>이후 반복 중단</button></div></article>)}
 {stopping&&<form className="editor" onSubmit={async e=>{e.preventDefault();if(await send('stop_routine',{routineId:stopping.id,fromDate:from}))setStopping(null);}}><h4 className="task-title" data-typo-role="title">{stopping.title} · 이후 반복 중단</h4><DateField label="중단 기준 날짜" required min={stopping.start_date} max={stopping.end_date} value={from} onChange={setFrom}/><p>중단 {stopTargets.length}개 · 보존 {(stopping.occurrences.filter(t=>t.date>=from).length-stopTargets.length)}개</p><p className="form-help">보존할 기록은 유지하며 중단한 날짜는 휴지통에서 복원할 수 있습니다.</p><div className="actions"><button className="link" type="button" onClick={()=>setStopping(null)}>취소</button><button disabled={locked}>이후 반복 중단</button></div></form>}
 </section>;
}
