...
postgres/_supabase 백업과 격리 복구, 앱 16개·내부 5개 테이블 대조, 매일 자동 백업 설정 및 첫 실행을 완료했다. 실제 VM 재부팅·별도 장비 복제·전체 플랫폼 재구성은 미실행. 근거: [운영 검증](BACKUP-RESTORE-2026-09-21.md).


## 2026-09-21 VM 재부팅 검증 완료

사용자 승인 후 Ubuntu VM 정상 재부팅. 서비스 12개 자동 재시작·healthy, 자료 revision 32 및 16개 테이블 보존, 공개 파일 23개 일치, 관리자 보호와 자동 백업 예약 정상. [실제 검증](REBOOT-VERIFICATION-2026-09-21.md). 과거 재부팅 미완료 문장은 당시 이력이다.
