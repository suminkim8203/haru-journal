...
기존 NAS 0001~0006 적용, snapshot schemaVersion 4. haru_snapshot/haru_command/haru_export는 고정 단일 공개 diary만 접근한다. 내부 테이블과 임의 diary 호출은 막혀 있다. 이미 적용한 SQL을 다시 실행하거나 기존 DB를 초기화하지 않는다. Studio 관리 경로와 앱 API 루트는 다르다. 실제 값은 Git·소스·문서에 기록하지 않는다.

[최신 구현/실제 검증/한계](../docs/implementation/SUPABASE-BACKEND-2026-09-18.md). 루틴·일부 UI·영속 자료 복원 증거·공개 웹 앱 배포는 미완료이며 DB 서비스 설치/앱 연결/배포를 구분한다.
