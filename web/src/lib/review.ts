...
import {dayWindow,runWork,seoulDate,weekDays} from './schedule.ts';
...
export function planResults(data:Snapshot,planId:string,period:'all'|'week',today=seoulDate()){
 ...
 const tasks=data.tasks.filter(t=>general.has(t.plan_id)&&(period==='all'||!!t.due_date&&t.due_date>=days[0]&&t.due_date<=days[6]));
 ...
 const expected=tasks.reduce((n,t)=>n+t.estimated_minutes*60000,0),actual=runs.reduce((n,r)=>n+runWork(r),0);
...
export function duration(ms:number){const sign=ms<0?'−':'',total=Math.floor(Math.abs(ms)/60000);return sign+(total>=60?`${Math.floor(total/60)}시간 ${total%60}분`:`${total}분`);}
