...
import fcntl
restore_lock=open("/backup/haru-backup-mutation.lock","a")
if os.environ.get("HARU_BACKUP_LOCK_HELD")!="1":fcntl.flock(restore_lock,fcntl.LOCK_SH)
...
try:
 ...
 manifest=root/'integrity-state.json'
 expected=json.loads((manifest if manifest.exists() else root/'snapshot-before.json').read_text())
 if expected.get('backupStateVersion') in (2,3):
  assert expected['backupStateVersion']==3,'Version 2 comparison is locale-dependent; create a canonical backup before verification'
  restored=journal_state(lambda q:sql(q).stdout.decode().strip())
  assert restored==expected,'Backup journal state differs from saved manifest; do not resume service'
  report['allJournalTablesExact']=True
  report['journalTables']=len(restored['tables'])
  report['counts']={k:v['rows'] for k,v in restored['tables'].items()}
  private_schema=sql("select to_regclass('journal.account_diaries') is not null;").stdout.decode().strip()=='t'
  report['requiresAuthMigrationBeforeProduction']=not private_schema
  if private_schema:
   report['permissionBoundary']=sql("select not has_function_privilege('anon','public.haru_snapshot()','EXECUTE') and not has_schema_privilege('anon','journal','USAGE');").stdout.decode().strip()=='t'
   assert report['permissionBoundary'],'Private access boundary changed'
  else:report['permissionBoundary']=False
 else:
  ...
  report['allJournalTablesExact']=False
  report['legacySnapshotOnlyVerified']=True
  report['requiresCanonicalManifest']=True
  report['requiresAuthMigrationBeforeProduction']=True
