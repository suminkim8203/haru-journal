...
export function RunEditor({task,day,mode,run,onClose}:{task:Task;day:string;mode:'placement'|'run';run?:Run;onClose:()=>void}){
 ...
 return <form className="editor" onSubmit={async e=>{e.preventDefault();setError('');try{
  ...
  <p className="field-help">기록한 시작·종료 사이의 시간만 저장됩니다. 시간이 벗어나면 값을 유지하고 오류를 알려드립니다.</p>
  {pauses.map((p,i)=><div className="pause-editor" key={i}><label>유형<select value={p.kind} onChange={e=>setPauses(pauses.map((v,j)=>j===i?{...v,kind:e.target.value as 'break'|'interrupt'}:v))}><option value="break">휴식</option><option value="interrupt">중단</option></select></label>
   ...
   <DateField label="구간 종료일" min={date} max={endDate} required value={p.endDate} onChange={value=>setPauses(pauses.map((v,j)=>j===i?{...v,endDate:value}:v))}/><TimeField min={p.endDate===date?start:undefined} max={p.endDate===endDate?end:undefined} label="구간 종료" value={p.end} onChange={value=>setPauses(pauses.map((v,j)=>j===i?{...v,end:value}:v))}/><button type="button" className="text-button" onClick={()=>setPauses(pauses.filter((_,j)=>j!==i))}>제거</button></div>)}
  <label>막힌 이유 · 선택<textarea className="record-input" maxLength={2000} value={reason} onChange={e=>setReason(e.target.value)} placeholder="실행 중 막힌 이유가 있었다면 남겨 주세요."/></label></>}
  {error&&<p role="alert">{error}</p>}<div className="actions"><button className="primary" type="submit">{locked?'저장 중…':mode==='placement'?'배치 저장':'실행 기록 저장'}</button></div></fieldset>
 </form>;
}
